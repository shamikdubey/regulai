  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <motion.div
        ...
        className="mb-6 rounded-2xl bg-gradient-to-r from-[#eff6ff] via-[#f0f9ff] to-[#eff6ff] border border-[#bfdbfe] p-6"
      >
      ...
      >
        ...
        {projectsQ.isLoading ? (
          <div className="h-32 bg-white border border-[#e2e8f0] rounded-xl flex items-center justify-center">
            <span className="text-xs text-[#94a3b8]">Loading projects…</span>
          </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-[#0f172a]">No active projects</p>
              <p className="text-[10px] text-[#94a3b8] mt-0.5">
                Track gap analysis, checklist, documents, and compliance review in one place
              </p>
            </div>
            <span className="text-xs font-bold text-[#2563eb] flex items-center gap-1 flex-shrink-0 group-hover:gap-2 transition-all">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {allProjects.slice(0, 3).map((proj: any) => {
              ...
              const domainColor  = DOMAIN_COLORS[proj.domain] ?? "#64748b";
              const activeStep   = getActiveStepIndex(proj.progress);
              return (
                <button
                  key={proj.id}
                  onClick={() => navigate(`/projects/${proj.id}`)}
                  className="text-left p-4 rounded-xl bg-white border border-[#e2e8f0] hover:border-[#cbd5e1] hover:bg-[#f8fafc] transition-all shadow-sm group flex flex-col gap-3"
                >
                  {/* Name + domain */}
                  <div>
                    <p className="text-xs font-bold text-[#0f172a] truncate leading-snug mb-1">
                      {proj.product_name}
                    </p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-bold text-[#2563eb] flex items-center gap-1 group-hover:gap-1.5 transition-all">
      ...
      >
        <div className="grid grid-cols-3 gap-4">
          ...
          <button
            ...
            className="flex items-center gap-4 p-5 rounded-2xl bg-[#2563eb] text-white hover:bg-[#1d4ed8] active:scale-[0.98] transition-all shadow-sm group"
          >
          ...
          <button
            ...
            className="flex items-center gap-4 p-5 rounded-2xl bg-white border border-[#e2e8f0] hover:border-[#2563eb] hover:bg-[#f8fafc] active:scale-[0.98] transition-all shadow-sm group"
          >
            <div className="w-10 h-10 rounded-xl bg-[#eff6ff] flex items-center justify-center flex-shrink-0">
              <MessageSquare size={20} className="text-[#2563eb]" />
            </div>
            <div className="text-left min-w-0">
              <p className="text-sm font-bold text-[#0f172a] leading-snug">Ask RegulAI</p>
              <p className="text-[11px] text-[#94a3b8] mt-0.5">Query any regulation</p>
            </div>
            <ArrowRight
              size={16}
              className="ml-auto text-[#cbd5e1] group-hover:text-[#2563eb] flex-shrink-0 transition-colors"
            />
          </button>
          ...
          <button
            ...
            className="flex items-center gap-4 p-5 rounded-2xl bg-white border border-[#e2e8f0] hover:border-[#f59e0b] hover:bg-[#fffbeb] active:scale-[0.98] transition-all shadow-sm group"
          >
            ...
            <div className="text-left min-w-0">
              <p className="text-sm font-bold text-[#0f172a] leading-snug">Check Alerts</p>
      ...
      <motion.div
        ...
        className="mb-8 bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden shadow-sm"
      >
            ...
            {previewAlerts.map((a: any) => {
              ...
              return (
                <div
                  key={a.id}
                  onClick={() => navigate("/alerts")}
                  className="px-5 py-3.5 hover:bg-[#f8fafc] transition-colors cursor-pointer flex items-start gap-3"
                >
                  <span
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-semibold text-[#0f172a] truncate">{a.title}</p>
                  ...
                  <span className="text-[10px] font-mono text-[#94a3b8] flex-shrink-0 mt-0.5">
                    {a.published_at
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {STAT_CONFIG.map((s, i) => (
          ...
          >
            <div className="text-[10px] font-mono text-[#94a3b8] uppercase tracking-widest mb-2">
              {s.label}
            </div>
      ...
      <div className="mb-8">
        <h2 className="text-xs font-mono text-[#94a3b8] uppercase tracking-widest mb-3">
          Quick Actions
        </h2>
        <div className="grid grid-cols-5 gap-3">
          {QUICK_ACTIONS.map((a, i) => (
            ...
            >
              ...
              <p className="text-xs font-semibold text-[#0f172a] leading-snug mb-1">
                {a.label}
              </p>
        ...
        >
          ...
          {recentQueries.length === 0 ? (
            <div className="flex flex-col items-center py-10 text-[#94a3b8]">
              ...
              <button
                onClick={() => navigate("/query")}
                className="text-[10px] text-[#2563eb] mt-2 hover:underline"
              >
            ...
            <div className="divide-y divide-[#e2e8f0]">
              {recentQueries.map((q) => {
                ...
                return (
                  <div
                    key={q.id}
                    className="px-5 py-3 hover:bg-[#f8fafc] transition-colors"
                  >
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs text-[#0f172a] leading-snug truncate">
                          {a.title}
                        </p>
                        <div className="flex items-center gap-2 mt-0.5 text-[10px] font-mono text-[#94a3b8]">
                          {j && <span>{j.flag} {j.label}</span>}
