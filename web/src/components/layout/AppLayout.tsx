...
export default function AppLayout() {
  ...
  const SidebarContent = ({ collapsed }: { collapsed: boolean }) => (
    <>
      ...
      <div className={cn(
        ...
        {!collapsed && (
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[9px] text-[#4a5568] font-mono tracking-widest">COMPLIANCE AI</div>
          </div>
        )}
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        {NAV.filter(({ adminOnly }) => !adminOnly || user?.role === "admin").map(({ to, icon: Icon, label, group, badge }) => {
          ...
          return (
            <div key={to}>
              {showGroup && (
                <div className="px-3 pt-3 pb-1 text-[9px] font-mono text-[#2a3040] uppercase tracking-widest">
                  {group}
                </div>
              )}
              <NavLink to={to}>
      ...
      {/* User */}
      <div className={cn(
        "border-t border-[#1f2530] p-2 flex items-center gap-2 flex-shrink-0",
        collapsed && "justify-center"
      )}>
        ...
        {!collapsed && (
          <>
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[10px] text-[#4a5568] truncate">{tenant?.name}</div>
            </div>
            <button
        ...
        {collapsed && (
          <button
            onClick={handleLogout}
            className="text-[#4a5568] hover:text-[#ff4757] transition-colors absolute bottom-2 right-2 opacity-0 group-hover:opacity-100"
            title="Sign out"
          >
          </button>
        )}
  ...
  return (
    <div className="flex h-screen bg-[#0a0c10] text-[#e8ecf2] overflow-hidden">
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 h-full w-72 bg-[#111318] border-r border-[#1f2530] z-50 lg:hidden flex flex-col"
            >
      ...
      <motion.aside
        ...
        className="hidden lg:flex flex-shrink-0 flex-col bg-[#111318] border-r border-[#1f2530] overflow-hidden"
      >
      ...
      <main className="flex-1 overflow-hidden flex flex-col min-w-0">
        ...
        <header className="h-12 border-b border-[#1f2530] flex items-center px-4 gap-3 flex-shrink-0 bg-[#0a0c10]">
          ...
          {/* Version badge */}
          <span className="text-[10px] text-[#4a5568] font-mono hidden sm:block">
